...
// Create or list reports depending on method
export async function GET(
  ...
  try {
    ...
    // Return empty list for now; UI handles "no reports" state
    return NextResponse.json([]);
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    // Simulate a created report id
    const reportId = `${params.id}-${Date.now()}`;
    return NextResponse.json({
      success: true,
      id: reportId,
